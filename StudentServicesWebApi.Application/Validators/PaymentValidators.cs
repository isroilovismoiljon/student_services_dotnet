...
public class ProcessPaymentDtoValidator : AbstractValidator<ProcessPaymentDto>
{
    public ProcessPaymentDtoValidator()
    {
        ...
        // ApprovedAmount validation when status is Success (optional - defaults to RequestedAmount if null)
        ...
        // ApprovedAmount should be null when status is Rejected
        RuleFor(x => x.ApprovedAmount)
        ...
        // RejectReason should be null when status is Success
        RuleFor(x => x.RejectReason)
